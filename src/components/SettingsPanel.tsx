  ...
  const handleImportSupabaseJSON = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (event) => {
      try {
        ...
        setSyncLoading(true);
        const { getSupabaseClient } = await import('../lib/supabaseAuth');
        ...
        for (const mut of parsed.mutations) {
          try {
            const { error } = await supabase.from('token_mutations').insert({
              id: mut.id || undefined,
              timestamp: mut.timestamp,
              remaining_kwh: mut.remaining_kwh || mut.remainingKwh,
            ...
            <div className="border-t border-slate-100 dark:border-slate-800 pt-4 space-y-4">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    type="button"
                    title="Ekspor Kredensial JSON"
                    aria-label="Ekspor Kredensial JSON"
                    onClick={handleExportSupabaseCredentials}
                    className="p-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-emerald-500 rounded-lg transition-colors cursor-pointer shadow-sm border border-slate-200 dark:border-slate-700"
                  >
                    <Download className="h-3.5 w-3.5" />
                  </button>
                  <label 
                    title="Impor Kredensial JSON"
                    aria-label="Impor Kredensial JSON"
                    className="p-1.5 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-indigo-500 rounded-lg transition-colors cursor-pointer shadow-sm border border-slate-200 dark:border-slate-700 flex items-center justify-center"
                  >
              ...
              {onMirrorAllToSupabase && (
                <div className="border-t border-emerald-100/50 dark:border-emerald-900/30 pt-3 mt-1.5 space-y-2">
                  ...
                  <div className="flex flex-wrap gap-2.5 pt-1">
                    ...
                    <label className={`px-4 py-2 ${syncLoading || isLoading ? 'bg-indigo-700/50 cursor-not-allowed' : 'bg-indigo-600 hover:bg-indigo-700 cursor-pointer'} text-white font-bold text-xs rounded-xl flex items-center gap-2 transition-all shadow-xs`}>
                      <Upload className="h-3.5 w-3.5" />
                      <span>Impor Data JSON ke Supabase</span>
                      <input 
                        type="file" 
                        accept=".json" 
                        disabled={syncLoading || isLoading}
                        onChange={handleImportSupabaseJSON} 
                        className="hidden" 
                      />
                    </label>
                  </div>
